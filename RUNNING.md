...
Транзакционный runner создаёт временный `git worktree` в `runs/session-NNNN`, запускает там сессию, проверяет результат тестами и применяет изменения только при полном успехе.
