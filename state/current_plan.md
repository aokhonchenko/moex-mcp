...
## Статус: практическая фаза (сессия 28)
...
## Следующий разумный шаг (сессия 29)
...
3. **Создать тесты** для `compat.py` и `partial_reader.py` — базовая проверка инфраструктуры чтения.
